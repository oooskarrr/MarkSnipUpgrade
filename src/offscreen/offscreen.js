...
async function handleContextMenuDownload(info, tabId, providedOptions = null) {
  ...
  try {
    ...
    const article = await getArticleFromContent(tabId, 
      info.menuItemId === "download-markdown-selection" || info.menuItemId === "download-markdown-selection-no-links",
      options
    );
    if (!article?.content) {
    ...
    await downloadMarkdown(finalMarkdown, title, tabId, imageList, mdClipsFolder, options);
...
async function handleContextMenuCopy(info, tabId, providedOptions = null) {
  ...
  if (info.menuItemId === "copy-markdown-link") {
    ...
    const article = await getArticleFromContent(tabId, info.menuItemId === "copy-markdown-selection" || info.menuItemId === "copy-markdown-selection-no-links", options);  // Added options
    const { markdown } = await convertArticleToMarkdown(article, false, options);
    ...
    await executeScriptInTab(tabId, `copyToClipboard(${JSON.stringify(finalMarkdown)})`);
  }
}
