...
async function handleDownloadRequest(message) {
  ...
  // Strip links and images if requested
  if (message.stripLinksAndImages) {
    markdown = stripLinksAndImages(markdown);
  }
...
async function downloadMarkdownFromContext(info, tab) {
  ...
  if (typeof chrome !== 'undefined' && chrome.offscreen) {
    ...
    const article = await getArticleFromContent(tab.id, info.menuItemId == "download-markdown-selection" || info.menuItemId == "download-markdown-selection-no-links");
    const title = await formatTitle(article);
    const { markdown, imageList } = await convertArticleToMarkdown(article);
    
    // Strip links and images if this is a "no-links" menu item
    const finalMarkdown = (info.menuItemId.endsWith("-no-links")) ? stripLinksAndImages(markdown) : markdown;
    
    const mdClipsFolder = await formatMdClipsFolder(article);
    await downloadMarkdown(finalMarkdown, title, tab.id, imageList, mdClipsFolder);
  }
}
  ...
  if (typeof chrome !== 'undefined' && chrome.offscreen) {
    ...
    try {
      ...
      if (info.menuItemId == "copy-markdown-link") {
        ...
        const article = await getArticleFromContent(tab.id, info.menuItemId == "copy-markdown-selection" || info.menuItemId == "copy-markdown-selection-no-links");
        const { markdown } = await convertArticleToMarkdown(article, false);
...
/**
 * Strip links and images from markdown text
 */
